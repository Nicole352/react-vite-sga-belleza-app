...
const GestionCursos = () => {
  ...
  // Estados para control de fechas
  ...
  const handleTipoCursoChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (tipoId && (modalType === 'create' || modalType === 'edit')) {
      ...
      if (tipo) {
        ...
        if (fechaInicio && tipo.duracion_meses) {
          calculateAndSetFechaFin(fechaInicio, tipo.duracion_meses);
        }
      }
    }
  };

  const calculateAndSetFechaFin = (start: string, months: number) => {
    ...
    try {
      ...
      setFechaFin(`${newYear}-${newMonth}-${newDay}`);
      showToast.info(`Fecha de fin calculada automáticamente: ${months} meses de duración`, darkMode);
    } catch (e) {
  ...
  const handleFechaInicioChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Si hay un tipo seleccionado, intentar calcular fecha fin
    if (selectedTipoCurso) {
      const tipo = tiposCursos.find(t => t.id_tipo_curso === selectedTipoCurso);
      if (tipo && tipo.duracion_meses) {
        calculateAndSetFechaFin(newFechaInicio, tipo.duracion_meses);
      }
    } else if (modalType === 'edit' && selectedCurso) {
      // En edición, si no se ha cambiado el tipo, usar el del curso actual
      const tipo = tiposCursos.find(t => t.id_tipo_curso === selectedCurso.id_tipo_curso);
      if (tipo && tipo.duracion_meses) {
        calculateAndSetFechaFin(newFechaInicio, tipo.duracion_meses);
      }
    }
  ...
  const handleEditCurso = async (curso: Course) => {
    ...
    setAutoGeneratedCode('');
    setAutoGeneratedName('');
    setFechaInicio(finalCurso.fecha_inicio);
    setFechaFin(finalCurso.fecha_fin);
    setShowModal(true);
    setLoading(false);
  };
  ...
  const handleViewCurso = async (curso: Course) => {
    ...
    if (cursoCompleto) {
      ...
      setModalType('view');
      setFechaInicio(cursoCompleto.fecha_inicio);
      setFechaFin(cursoCompleto.fecha_fin);
      setShowModal(true);
    } else {
      // Fallback: usar el curso de la lista si falla la carga
      setSelectedCurso(curso);
      setModalType('view');
      setFechaInicio(curso.fecha_inicio);
      setFechaFin(curso.fecha_fin);
      setShowModal(true);
    }
  };
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isSmallScreen ? '1fr' : '1fr 1fr', rowGap: isMobile ? '0.75rem' : '1rem', columnGap: isSmallScreen ? 0 : '1.25rem' }}>
                  ...
                  <div>
                    ...
                    {modalType === 'edit' || modalType === 'view' ? (
                      // En edición o visualización: mostrar solo el nombre del tipo (no editable)
                      <div style={{
                        width: '100%',
                        padding: '0.75rem',
                        background: darkMode ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)',
                  ...
                  <div>
                    ...
                    <input
                      ...
                      defaultValue={modalType === 'edit' || modalType === 'view' ? selectedCurso?.nombre : undefined}
